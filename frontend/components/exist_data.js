...
export default function ExistData({
  loading,
  setLoading,
  setErrorStatus,
  setErrorMessage,
  ...
  async function _handleNewData() {
    setLoading(true);
    try {
      const res = await axios.post(`http://localhost:8080/existing_data`, {
        stemming: isStemming,
  ...
  return (
    <div className="text-left block p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
      ...
      <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
        Retrieve a dataset that has existed since the beginning of the initial
        data.
      </p>
      ...
      <div className="flex items-center justify-between">
        ...
        <label
          htmlFor="old_data_stemming"
          className="inline-flex relative items-center cursor-pointer"
        >
          <input
            type="checkbox"
            value=""
            id="old_data_stemming"
